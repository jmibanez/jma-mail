...
- **`db_only`** -- a row whose `<folder>/cur` is gone from disk (an orphaned cache row).
- **`config_dropped`** -- a folder present on both sides whose mailbox is no longer selected by `[sync].mailboxes`. Computed with the same path-based `jmap::mailbox::get_selected_mailboxes` the engine uses, so `status`/`prune` agree with what `sync` would actually select. Empty when the config syncs every mailbox.
