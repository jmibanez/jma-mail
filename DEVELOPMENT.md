...
- `src/state/` -- SQLite. Schema defined inline in `db.rs`; version constant is `db.rs::SCHEMA_VERSION`. Tables:
  - `jmap_state` -- per-entity sync cursor, one row per `(account, entity)`. `("acct", "Email")` ratchets in `execute`; `("acct", "Mailbox")` is stamped at the cycle tail from the `Mailbox/get` state, gated on the cycle's staged `mailbox_map` writes having landed so the cursor never advances past a cache that is still catching up. Both seed the daemon's SSE state-change dedup cache so the first event after connect isn't a redundant trigger.
