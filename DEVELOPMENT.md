...
- `src/domain.rs` -- neutral domain types shared across layers (`MailboxObject`, `MailboxFolderBinding`, `MaybeReference`). Depends only on `ids` + `std`; produced by `jmap`, consumed by `maildir_ops` and `sync`, so it lives below both peers rather than inside either (see [Data model](#data-model)).
