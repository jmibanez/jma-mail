...
This section configures which Maildirs `jma` will sync to, and how it syncs. The important knobs here are `maildir_path` which should point to the root of your Maildir mailboxes that you want to sync (e.g. `~/Mail/Fastmail`, or `~/Mail/my-provider`). By default `jma init` populates `mailboxes` with the common IMAP/JMAP mailboxes (INBOX, Archive, Sent, Drafts, Trash) and none of your custom folders/mailboxes -- if you want to populate _all_ mailboxes from upstream, unset this key or set it to an empty list. The Maildir tree itself is created on the first sync, one folder per server-known mailbox.
  ...
  * `mailboxes`: The specific mailboxes you want to sync. Set this to `[]` (an empty list) or leave this empty to sync all mailboxes. Entries match a mailbox by its full server path, root-first and slash-joined (e.g. `[Gmail]/Sent`), and a matched path pulls in its whole subtree, so naming a parent syncs everything under it. A bare leaf name therefore selects only a top-level mailbox; to pick a nested one, write out its full path. The literal `INBOX` is the exception: it matches whichever mailbox has the inbox role regardless of its localized name (e.g. `Posteingang`), which is always stored locally as `INBOX`.
  * `case_insensitive_match`: When comparing `mailboxes` entries against the server's mailbox paths, the cases can differ (e.g. on a case-insensitive, case-preserving filesystem your mailbox is named `foo` but upstream is saved as `Foo`). Set this to `true` to ignore case; default is to respect case.
