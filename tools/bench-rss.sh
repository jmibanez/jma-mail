...
#
# Usage:
#   tools/bench-rss.sh <before-binary> <after-binary> \
#                      <maildir-source> <account-email>
#
# All four positional arguments are required and have no defaults;
# they identify the binaries to compare, the maildir to copy into
# the scratch dir, and the email address to write into the test
# config.
#
# Environment overrides:
#   BENCH_DIR  Scratch dir for maildir copy, state DB, logs.
#              Default: /tmp/jmapsync-bench
#   SUBCMD     jmapsync subcommand to benchmark.
#              Default: pull
#
# Setup is idempotent: the maildir is copied and the config written
# only if missing. Delete $BENCH_DIR to force a fresh setup.
#
...
usage() {
    cat >&2 <<EOF
usage: $0 <before-binary> <after-binary> <maildir-source> <account-email>
...
for bin in "$BEFORE_BIN" "$AFTER_BIN"; do
    if [[ ! -x "$bin" ]]; then
